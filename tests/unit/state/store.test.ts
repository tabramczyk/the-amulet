...
describe('Game Store', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should have prestige initialized', () => {
      const state = store.getState();
      expect(state.prestige.livesLived).toBe(0);
      expect(state.prestige.totalDaysAllLives).toBe(0);
    });
  });
  ...
  describe('resetForReincarnation', () => {
    ...
    it('should increment lives lived', () => {
      store.getState().resetForReincarnation();
      expect(store.getState().prestige.livesLived).toBe(1);
    });

    it('should accumulate total days across lives', () => {
      store.getState().advanceDays(1000);
      store.getState().resetForReincarnation();
      expect(store.getState().prestige.totalDaysAllLives).toBe(1000);
    });

    it('should accumulate skill levels into prestige', () => {
      store.getState().updateSkill('concentration', { level: 10 });
      store.getState().resetForReincarnation();
      const prestige = store.getState().prestige.skillPrestige.find(
        (sp) => sp.skillId === 'concentration',
      );
      expect(prestige?.totalLevelsAllLives).toBe(10);
    });

    it('should accumulate job levels into prestige', () => {
      store.getState().updateJob('beggar', { level: 20 });
      store.getState().resetForReincarnation();
      const prestige = store.getState().prestige.jobPrestige.find(
        (jp) => jp.jobId === 'beggar',
      );
      expect(prestige?.totalLevelsAllLives).toBe(20);
    });
    ...
    it('should stack prestige across multiple lives', () => {
      ...
      expect(prestige?.totalLevelsAllLives).toBe(27);
      expect(store.getState().prestige.livesLived).toBe(2);
      expect(store.getState().prestige.totalDaysAllLives).toBe(800);
    });
  ...
  describe('resetGame', () => {
    it('should reset everything including prestige', () => {
      store.getState().updateSkill('concentration', { level: 10 });
      store.getState().resetForReincarnation();
      store.getState().resetGame();

      expect(store.getState().prestige.livesLived).toBe(0);
      expect(store.getState().prestige.totalDaysAllLives).toBe(0);
    });
  });
